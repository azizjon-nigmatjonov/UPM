...
const DraggableTable = ({
  ...
  const updateSubtaskOrder = ({ removedIndex, addedIndex }) => {
    subtaskService.updateOrder(lists[removedIndex]._id, addedIndex + 1);
  };
  ...
  const fetchMore = (page) => {
    if(count <= lists.length) setHasMore(false)
    else setPage(page + 1);
  };
  ...
  return (
    <div style={{ padding: 8 }}>
      ...
      <div className='ListTable' id='scrollableDiv'>
              ...
              {lists?.map((list, idx) => (
                <Draggable key={list._id}>
                  <div
                    className='table-row'
                    onClick={() => {
                      setSelectedSubtask({ ...list, subtask_id: list._id });
                  ...
                  >
                    ...
                    {editingSubtask !== list._id ? (
                      <div
                        className='table-cell'
                        onClick={(e) => {
                          e.stopPropagation();
                          setEditingSubtask(list._id);
                          setEditingDescription(null);
                          setSelectedSubtask({ ...list, subtask_id: list._id });
                          formik.setValues({
                            ...list,
                    ...
                    {editingDescription !== list._id ? (
                      <div
                        className='table-cell description-cell'
                        onClick={(e) => {
                          e.stopPropagation();
                          setEditingSubtask(null);
                          setEditingDescription(list._id);
                          setSelectedSubtask({ ...list, subtask_id: list._id });
                          formik.setValues({
                            ...list,
                            title: list.title,
                    ...
                    >
                      <DateTimePicker
                        ...
                        onChange={(val) => updateSubtask(val, 'end_time')}
                    ...
                    <div className='table-cell'>
                      <RectangleIconButton
                        loader={deleteLoader}
                        color='error'
                        onClick={() => deleteSubtask(list?._id)}
                      >
                        <Delete color='error' />
                      </RectangleIconButton>
                    </div>
